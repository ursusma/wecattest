...
def lnr(request):
    ...
    def post(request):
        ...
        if test == 'text':
            ...
            if xmlData.find('Event').text == 'CLICK':
                if xmlData.find('EventKey').text == 'v1001_joke':
                    tex = u'笑话'
                    text = getjson(tex)
                    res = retext(touser,fromuser,text)
                elif xmlData.find('EventKey').text == 'v1001_news':
                    res = renews(touser,fromuser)
            else:
